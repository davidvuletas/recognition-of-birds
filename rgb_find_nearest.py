...
class ColorNames:
    ...
    def findNearestWebColorName(points):
        return ColorNames.findNearestColorName(points, ColorNames.WebColorMap)
...
def detectColors(img):
    ...
    for x in range(0, w):
        for y in range(0, h):
            nearest_color = ColorNames.findNearestWebColorName(img[x][y])
            if nearest_color in colors:
...
def removeMostShownColors(colors, img):
    print(colors)
...
if __name__ == "__main__":
    img = cv2.imread('D:\Workspace\Pycharm\SOFT\pictures\p1.jpg')
    # cv2.imshow('bef',img)
    colors = {}
    img = cv2.resize(img, (100, 100))
    start_time = time()
    colors = detectColors(img)
    colors = OrderedDict(sorted(colors.items(), key=lambda x: x[1],reverse=True))
    colors = dict(itertools.islice(colors.items(), 2))
    img = removeMostShownColors(colors,img)
    cv2.imwrite('D:\Workspace\Pycharm\SOFT\pictures\pp.jpg',img)
    print(time()-start_time)
#
# hex = ColorNames.WebColorMap[nearest_color]
# hex = hex.lstrip('#')
